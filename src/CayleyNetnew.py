...
def jacobi_method(A, b, K):
    ...
    d = diag_inv @ b # elementwise multiplication of diag_inv by each column of b
...
class CayleyConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, r, normalization='sym', jacobi_iterations=10, sparse=False):
        ...
        self.c = nn.ModuleList([torch.nn.Linear(in_channels, out_channels, device = device, bias = False).to(torch.cfloat) for _ in range(r)]) # c parameter
    ...
    def forward(self, x: torch.tensor, edge_index: torch.tensor, edge_weight: torch.tensor = None):
        ...
        # A = (hL + iI),  b = (hL - iI)x
        y_i = torch.complex(x.clone(), torch.zeros(x.size(), device = device))
